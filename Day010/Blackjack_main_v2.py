def check_dealer_ace(reveal_dhand):
  while sum(reveal_dhand) > 21 and 11 in reveal_dhand:
    for i in range(len(reveal_dhand)):
      if reveal_dhand[i] == 11:
        reveal_dhand[i] = 1 
        check_dealer_ace(reveal_dhand)

  return reveal_dhand
def check_blackjack(reveal_uhand):
  while sum(reveal_uhand) > 21 and 11 in reveal_uhand:
    for i in range(len(reveal_uhand)):
      if reveal_uhand[i] == 11:
        reveal_uhand[i] = 1
        check_blackjack(reveal_uhand)  
  if sum(reveal_uhand) == 21:
    print(f"{reveal_uhand}Congratulations, you hit blackjack!")
  else:
...
while should_continue:
  ...
  reveal_dhand = check_dealer_ace(reveal_dhand)
  check_blackjack(reveal_uhand)
